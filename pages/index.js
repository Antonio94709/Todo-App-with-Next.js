...
export default function Home() {
  ...
  return (
    <div className="w-3/4 mx-auto text-center">
      ...
      <ul className="m-4 bg-white rounded p-5 text-center">
        {apitasklist.map((task, i) => {
          return (
            <div key={i}>
              <li className="w-full border-t p-3 text-black grid grid-cols4 gap-4 text-xl">
                {task.text}
                <button
                  onClick={() => removeApiTask(task.id)}
                  className="bg-black ml-7 02 text-white p-4 rounded flex-auto w-42"
                >
                  Remove item
                </button>
              </li>
